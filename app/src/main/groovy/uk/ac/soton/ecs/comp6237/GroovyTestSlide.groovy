...
public class GroovyTestSlide implements Slide {
	@Override
	public Component getComponent(int width, int height) throws IOException {
		...
		def font = Font.decode("Helvetica Neue-28")
		...
		def table = swing.panel(layout: new BorderLayout(), opaque:false) {
			widget(constraints:BorderLayout.CENTER, new ChartPanel(chart))
			panel(constraints:BorderLayout.WEST, opaque: false) {
				comboBox(id: 'cy', items: movies, actionPerformed: {
			...
			panel(constraints:BorderLayout.SOUTH, layout: new BorderLayout(), opaque: false) {
				comboBox(id: 'cx', constraints: BorderLayout.EAST, items: movies, actionPerformed: {
	...
	public static void main(String[] args) throws IOException {
		new SlideshowApplication(new GroovyTestSlide(), 1024, 768, Utils.BACKGROUND_IMAGE);
	}
}
